...
const BusinessCardPreview = ({ formData, template, customElements, customBackground }) => {
 ...
 // Templates
  const renderCustomTemplate = () => (
    <div 
      className="w-96 h-56 relative rounded-lg shadow-2xl overflow-hidden"
      style={{ backgroundColor: customBackground || '#ffffff' }}
    >
        ...
        >
          {element.type === 'text' ? (
            <div className="w-full h-full flex items-center">
              {element.id === 'name' ? formData.name || 'Your Name' :
               element.id === 'title' ? formData.title || 'Your Title' :
               element.id === 'company' ? formData.company || 'Company Name' :
               element.id === 'email' ? formData.email || 'email@example.com' :
               element.id === 'phone' ? formData.phone || '+1 (555) 123-4567' :
               element.content}
            </div>
          ) : (
            <QRCode value={formData.website} size={Math.min(element.width, element.height)} />
          )}
        </div>
      ))}
    </div>
    ...
    <div className="rounded-2xl  p-8">
      <div className="flex items-center justify-between mb-6">
       <h2 className="text-2xl font-bold text-gray-800 mb-6">Live Preview</h2>
          ...
          {template === 'custom' ? renderCustomTemplate() :
